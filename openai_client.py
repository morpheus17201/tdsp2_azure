...
async def get_openai_response(question: str, file_path: Optional[str] = None) -> str:
    ...
    """
    logger.info(f"Inside the function: get_openai_response")

    # Check for Excel formula in the question
    logger.info(f"Checking if there's excel formula in the question")
    ...
    logger.info(f"Checking if there's google sheets in the question")
    ...
    if (
        ...
        logger.info("Found")
        from functions import convert_keyvalue_to_json
    ...
    if (
        ...
        from functions import process_encoded_files
    ...
    try:
        ...
        logger.info(f"'message' in the response from OpenAI: {message}")

        # Check if there's a function call
        if "tool_calls" in message:
            logger.info(f"Detected tool_calls in 'message'")
            logger.info(f"Number of tool calls: {len(message['tool_calls'])}")
            for tool_call in message["tool_calls"]:

                function_name = tool_call["function"]["name"]
                logger.info(f"Function name returned by OpenAI: {function_name}")
